...
function logger(r)
  ...
  if user then
    ...
    -- benchmarks
    local time_proxy    = tostring(time_begin_proxy and math.floor((r:clock() - time_begin_proxy)/1000.0) or 0)
    local time_user_map = tostring(time_user_map and math.floor(time_user_map) or 0)
